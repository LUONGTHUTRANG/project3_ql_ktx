...
const RoomTable: React.FC<RoomTableProps> = ({
  ...
  const filteredRooms = rooms.filter((room) => {
    ...
    const matchStatus =
      !filterStatus || room.status.toLowerCase() === filterStatus.toLowerCase();
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mt-4 px-1 sm:px-0">
        <div className="flex flex-col gap-1 shrink-0">
          ...
          <p className="text-sm text-text-secondary dark:text-gray-400">
            Chọn phòng để xem chi tiết và quản lý
          </p>
        </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-800/50 border-b border-border-color dark:border-gray-700">
                    ...
                    <th className="p-4 text-xs font-semibold text-text-secondary dark:text-gray-400 uppercase tracking-wider text-right">
                      Trạng thái
                    </th>
                    {(userRole === "ADMIN" || userRole === "MANAGER") && (
                ...
                <tbody className="divide-y divide-border-color dark:divide-gray-700">
                  {paginatedRooms.length > 0 ? (
                    paginatedRooms.map((room) => (
                      <tr
                        key={room.id}
                        className={`group transition-colors ${editingRoomId === room.id ? "bg-amber-50 dark:bg-amber-900/20" : "hover:bg-gray-50 dark:hover:bg-gray-800/50"}`}
                      >
                        <td className="p-4">
                        ...
                        <td className="p-4 text-center">
                          {editingRoomId === room.id ? (
                            ...
                            <span className="text-sm text-text-main dark:text-white font-bold">
                              {room.max_capacity}
                            </span>
                          )}
                        </td>
                          ...
                          {editingRoomId === room.id ? (
                            ...
                            <>
                              {room.has_ac ? (
                                ...
                                <span className="material-symbols-outlined text-text-secondary/30 text-[20px]">
                                  close
                                </span>
                              )}
                            </>
                          ...
                          {editingRoomId === room.id ? (
                            <Checkbox
                              ...
                              onChange={(e) =>
                                setEditFormData({
                                  ...editFormData!,
                                  has_heater: e.target.checked ? 1 : 0,
                                })
                              }
                            />
                          ) : (
                            <>
                              {room.has_heater ? (
                                <span className="material-symbols-outlined text-orange-500 text-[20px] font-bold">
                                  hot_tub
                                </span>
                              ) : (
                                <span className="material-symbols-outlined text-text-secondary/30 text-[20px]">
                          ...
                          {editingRoomId === room.id ? (
                            <Checkbox
                              ...
                              onChange={(e) =>
                                setEditFormData({
                                  ...editFormData!,
                                  has_washer: e.target.checked ? 1 : 0,
                                })
                              }
                            />
                          ) : (
                            <>
                              {room.has_washer ? (
                                <span className="material-symbols-outlined text-blue-500 text-[20px] font-bold">
                    ...
                    <tr>
                      <td
                        colSpan={
                          userRole === "ADMIN" || userRole === "MANAGER" ? 9 : 8
                        }
