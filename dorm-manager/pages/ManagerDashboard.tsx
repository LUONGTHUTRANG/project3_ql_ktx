  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const occupancyRate =
          totalRooms > 0
            ? Math.round(((totalRooms - emptyRoomsCount) / totalRooms) * 100)
            : 0;
        ...
        const urgentCount = Array.isArray(requests)
          ? requests.filter((r) => r.type === "urgent" || r.status === "urgent")
              .length
          : 0;
        ...
        const notifResponse = await getNotificationsByManager(1, 3);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-surface-light dark:bg-surface-dark p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col gap-4 hover:shadow-md transition-shadow">
            ...
            <div>
              ...
              <p className="text-xs text-green-600 font-medium mt-1">
                +{stats.newStudentsThisMonth} sinh viên mới tháng này
              </p>
            </div>
          </div>
          <div className="bg-surface-light dark:bg-surface-dark p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col gap-4 hover:shadow-md transition-shadow">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">
                Tỷ lệ lấp đầy: {loading ? "..." : stats.occupancyRate}%
              </p>
            </div>
          </div>
          <div className="bg-surface-light dark:bg-surface-dark p-6 rounded-2xl shadow-sm border-l-4 border-l-orange-500 border-y border-r border-slate-200 dark:border-slate-700 flex flex-col gap-4 hover:shadow-md transition-shadow relative overflow-hidden">
            ...
            <div className="flex items-center justify-between z-10">
              ...
              <span
                className={`flex size-3 rounded-full animate-pulse ${stats.pendingRequests > 0 ? "bg-orange-500" : "bg-gray-300"}`}
              ></span>
            </div>
            <div className="z-10">
              <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">
              ...
              <p className="text-xs text-orange-600 font-medium mt-1">
                {stats.urgentRequests} yêu cầu khẩn cấp
              </p>
            </div>
          </div>
          <div className="bg-surface-light dark:bg-surface-dark p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col gap-4 hover:shadow-md transition-shadow">
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-slate-900 dark:text-white mt-1">
                {loading ? "..." : stats.maintenanceInProgress}
              </h3>
              <p className="text-xs text-slate-400 mt-1">
                Dự kiến xong: 16:00 hôm nay
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="flex-1 space-y-4">
              {loading ? (
                ...
                notifications.map((notif, index) => (
                  <div
                    key={notif.id || index}
                    className="flex items-start gap-4 p-4 rounded-xl bg-primary/10 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors border border-transparent"
                  >
          ...
          <div className="bg-surface-light dark:bg-surface-dark rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 flex flex-col">
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                ...
                <span
                  className="material-symbols-outlined text-blue-600 ml-auto"
                  style={{ fontSize: "20px" }}
                >
                  arrow_forward
                </span>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Báo hỏng, hỗ trợ sinh viên
                  </p>
                </div>
                <span
                  className="material-symbols-outlined text-orange-600 ml-auto"
                  style={{ fontSize: "20px" }}
                >
                  arrow_forward
                </span>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Gửi tin đến sinh viên tòa A1
                  </p>
                </div>
                <span
                  className="material-symbols-outlined text-purple-600 ml-auto"
                  style={{ fontSize: "20px" }}
                >
                  arrow_forward
                </span>
              ...
              <button
                ...
                className="flex items-center gap-3 p-4 rounded-xl bg-green-50 dark:bg-green-900/20 hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors text-left group border border-green-200 dark:border-green-800"
              >
                <div className="bg-white dark:bg-slate-700 p-2 rounded-lg shadow-sm text-green-600 group-hover:text-green-700">
                  <span className="material-symbols-outlined">
                    app_registration
                  </span>
                </div>
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Xem và duyệt đơn đăng ký ở KTX
                  </p>
                </div>
                <span
        ...
        <div className="bg-surface-light dark:bg-surface-dark rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
          <div className="px-6 py-5 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">
              Yêu cầu hỗ trợ mới nhất (Tòa A1)
            </h3>
            <button
