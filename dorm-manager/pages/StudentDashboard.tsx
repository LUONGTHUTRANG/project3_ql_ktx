...
const StudentDashboard: React.FC = () => {
  ...
  const [latestRequest, setLatestRequest] = useState<SupportRequest | null>(
    null,
  );
    ...
    const fetchStudentData = async () => {
      try {
        ...
        if (student?.current_room_id) {
          ...
          // Fetch roommates
          const roommatesResponse = await getStudentsByRoomId(
            student.current_room_id,
          );
          const roommates = roommatesResponse || [];
          setRoommates(
            Array.isArray(roommates)
              ? roommates.filter((r) => r.id !== user.id)
              : [],
          );
        }
    ...
    >
      <div className="mx-auto flex flex-col gap-6">
        ...
        <div className="rounded-xl bg-white dark:bg-surface-dark border border-border-color dark:border-gray-700 shadow-sm p-5 md:p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button
            className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-400 hover:bg-teal-100 dark:hover:bg-teal-900/30 transition-colors font-medium text-sm w-full md:w-auto"
              ...
              <div className="flex items-start justify-between">
                    ...
                    <p className="text-text-main dark:text-white text-2xl font-bold mt-0.5">
                      {loading
                        ...
                        : latestInvoice?.total_amount
                          ? formatPrice(Number(latestInvoice.total_amount))
                          : "N/A"}
                    </p>
                  </div>
                </div>
                <span
                  className={`inline-flex items-center rounded-md px-2.5 py-1 text-xs font-semibold ring-1 ring-inset ${
                    latestInvoice?.status === "PAID" || latestInvoice?.paid_at
              ...
              <div className="flex flex-col gap-3 pt-4 border-t border-dashed border-border-color dark:border-gray-700">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-text-main dark:text-white">
                    {latestInvoice?.invoice_code || "N/A"}
                  </span>
                </div>
                {latestInvoice?.title && (
                  <div className="flex justify-between text-sm">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-text-main dark:text-white">
                    {latestInvoice?.created_at
                      ? new Date(latestInvoice.created_at).toLocaleDateString(
                          "vi-VN",
                        )
                      : "N/A"}
                  </span>
            ...
            <div className="flex flex-col rounded-xl bg-white dark:bg-surface-dark border border-border-color dark:border-gray-700 shadow-sm overflow-hidden">
              <div className="p-5 flex flex-col items-center justify-center gap-3">
                ...
                <div className="flex flex-col gap-2 text-center">
                  <p className="text-text-main dark:text-white text-lg font-bold">
                    Không có yêu cầu sửa chữa
                  </p>
                  <p className="text-text-secondary dark:text-gray-400 text-sm">
                    Bạn chưa gửi yêu cầu sửa chữa nào. Nếu cần hỗ trợ, vui lòng
                    tạo yêu cầu mới.
                  </p>
                </div>
                <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="size-8 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center justify-center transition-colors">
                  <span className="material-symbols-outlined text-[#9ca3af] group-hover:text-primary">
                    arrow_forward
                  </span>
                </div>
              </div>
              <h3 className="text-text-main dark:text-white text-lg font-bold mb-2">
                Đăng ký lưu trú
              </h3>
              <p className="text-text-secondary dark:text-gray-400 text-sm mb-4 line-clamp-2">
                Gia hạn hợp đồng hiện tại hoặc đăng ký phòng mới cho kỳ tới.
              </p>
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="size-8 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center justify-center transition-colors">
                  <span className="material-symbols-outlined text-[#9ca3af] group-hover:text-orange-600">
                    arrow_forward
                  </span>
                </div>
              </div>
              ...
              <p className="text-text-secondary dark:text-gray-400 text-sm mb-4 line-clamp-2">
                Gửi yêu cầu sửa chữa điện, nước hoặc cơ sở vật chất trong phòng.
              </p>
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="size-8 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center justify-center transition-colors">
                  <span className="material-symbols-outlined text-[#9ca3af] group-hover:text-purple-600">
                    arrow_forward
                  </span>
                </div>
              </div>
              ...
              <p className="text-text-secondary dark:text-gray-400 text-sm mb-4 line-clamp-2">
                Xem lại các hóa đơn đã thanh toán và lịch sử giao dịch chi tiết.
              </p>
