...
const RoomFeeInvoiceDetail: React.FC = () => {
  ...
  useEffect(() => {
    const loadInvoices = async () => {
      try {
        ...
        const semId = parseInt(semesterId || '0');

        const filtered = allInvoices.filter(
          (inv: Invoice) => inv.type === 'ROOM_FEE' && inv.semester_id === semId
        );

        if (filtered.length > 0) {
          setSemesterName(`Kỳ ${semId}`);
        }
  ...
  return (
    <RoleBasedLayout 
      ...
      headerTitle={`Danh sách hóa đơn ${semesterName}`}
    >
        ...
        <div className="bg-white dark:bg-surface-dark p-4 rounded-xl shadow-sm border border-border-color dark:border-gray-700">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
            <div className="md:col-span-6">
              ...
              <Input
                placeholder="Nhập mã hóa đơn, số phòng hoặc tên sinh viên..."
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {paginatedInvoices.map((row) => (
                    <tr key={row.id} className="group hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-text-main dark:text-gray-300">{row.student}</span>
                      </td>
...
export default RoomFeeInvoiceDetail;
