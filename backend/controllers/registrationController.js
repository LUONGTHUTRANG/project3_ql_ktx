...
export const createRegistration = async (req, res) => {
  ...
  try {
    ...
    // 2. Check registration time limit (1 month before semester starts)
    const now = new Date();
    ...
    if (now < oneMonthBeforeStart) {
      await connection.rollback();
      const daysUntilOpen = Math.ceil((oneMonthBeforeStart - now) / (1000 * 60 * 60 * 24));
      return res.status(400).json({ 
        message: `Chưa đến thời gian đăng ký. Đăng ký mở vào ${oneMonthBeforeStart.toLocaleDateString('vi-VN')} (còn ${daysUntilOpen} ngày)` 
      });
    }
    ...
    if (now > semesterStart) {
      await connection.rollback();
      return res.status(400).json({ 
        message: "Đã hết thời gian đăng ký cho kỳ học này." 
      });
    }
